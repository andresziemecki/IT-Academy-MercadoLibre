...
func (user *User) Get() *utils.ApiError{
	...
	url := fmt.Sprintf("%s%d", utils.UrlUser, user.ID) //el SprintF permite formatear lo que hay en la url
	fmt.Println()
	...
	if err := json.Unmarshal(data, &user); err != nil { // Esta variable es local en el if
		return &utils.ApiError{
			Message: err.Error(),
			Status: http.StatusInternalServerError,
		}
